...
{
    esp_font = g_pSurface->CreateFont_();
    defuse_font = g_pSurface->CreateFont_();
    dropped_weapons_font = g_pSurface->CreateFont_();

    g_pSurface->SetFontGlyphSet(esp_font, "Arial", 11, 700, 0, 0, FONTFLAG_DROPSHADOW);
    g_pSurface->SetFontGlyphSet(defuse_font, "Arial", 15, 700, 0, 0, FONTFLAG_DROPSHADOW);
    g_pSurface->SetFontGlyphSet(dropped_weapons_font, "Arial", 10, 700, 0, 0, FONTFLAG_DROPSHADOW);
...
{
    esp_ctx.pl = pl;
    esp_ctx.is_enemy = pLocal->m_iTeamNum() != pl->m_iTeamNum();
    esp_ctx.is_visible = pLocal->CanSeePlayer(pl, HITBOX_CHEST);
...
{
    g_pSurface->DrawSetColor(esp_ctx.clr);
    g_pSurface->DrawOutlinedRect(esp_ctx.bbox.left, esp_ctx.bbox.top, esp_ctx.bbox.right, esp_ctx.bbox.bottom);
    g_pSurface->DrawSetColor(Color::Black());
    g_pSurface->DrawOutlinedRect(esp_ctx.bbox.left - 1, esp_ctx.bbox.top - 1, esp_ctx.bbox.right + 1, esp_ctx.bbox.bottom + 1);
    g_pSurface->DrawOutlinedRect(esp_ctx.bbox.left + 1, esp_ctx.bbox.top + 1, esp_ctx.bbox.right - 1, esp_ctx.bbox.bottom - 1);
}
//--------------------------------------------------------------------------------
void Visuals::Player::RenderName()
{
    ...
    if(MultiByteToWideChar(CP_UTF8, 0, info.szName, -1, buf, 128) > 0) {
        int tw, th;
        g_pSurface->GetTextSize(esp_font, buf, tw, th);

        g_pSurface->DrawSetTextFont(esp_font);
        g_pSurface->DrawSetTextColor(esp_ctx.clr);
        g_pSurface->DrawSetTextPos(esp_ctx.feet_pos.x - tw / 2, esp_ctx.head_pos.y - th);
...
void Visuals::Player::RenderHealth()
{
    ...
    g_pSurface->DrawSetColor(Color(red, green, 0, 255));
    g_pSurface->DrawFilledRect(x + 1, y + 1, x + w - 1, y + height - 2);
}
//--------------------------------------------------------------------------------
void Visuals::Player::RenderArmour()
{
    ...
    g_pSurface->DrawSetColor(Color(0, 50, 255, 255));
    g_pSurface->DrawFilledRect(x + 1, y + 1, x + w - 1, y + height - 2);
}
//--------------------------------------------------------------------------------
void Visuals::Player::RenderWeapon()
{
    ...
    if(MultiByteToWideChar(CP_UTF8, 0, weapon->GetCSWeaponData()->szWeaponName + 7, -1, buf, 80) > 0) {
        ...
        g_pSurface->DrawSetTextFont(esp_font);
        g_pSurface->DrawSetTextColor(esp_ctx.clr);
        g_pSurface->DrawSetTextPos(esp_ctx.feet_pos.x - tw / 2, esp_ctx.feet_pos.y);
        g_pSurface->DrawPrintText(buf, wcslen(buf));
    }
}
//--------------------------------------------------------------------------------
...
void Visuals::Misc::RenderCrosshair()
{
    ...
    g_pSurface->DrawSetColor(g_Options.color_esp_crosshair);

    int cx = w / 2;
    int cy = h / 2;

    g_pSurface->DrawLine(cx - 25, cy, cx + 25, cy);
    g_pSurface->DrawLine(cx, cy - 25, cx, cy + 25);
}
//--------------------------------------------------------------------------------
void Visuals::Misc::RenderWeapon(C_BaseCombatWeapon* ent)
{
    ...
    g_pSurface->DrawSetColor(g_Options.color_esp_weapons);
    g_pSurface->DrawLine(bbox.left, bbox.top, bbox.right, bbox.top);
    g_pSurface->DrawLine(bbox.left, bbox.bottom, bbox.right, bbox.bottom);
    g_pSurface->DrawLine(bbox.left, bbox.top, bbox.left, bbox.bottom);
    g_pSurface->DrawLine(bbox.right, bbox.top, bbox.right, bbox.bottom);
    ...
    if(MultiByteToWideChar(CP_UTF8, 0, name, -1, buf, 80) > 0) {
        ...
        g_pSurface->DrawSetTextFont(esp_font);
        g_pSurface->DrawSetTextColor(g_Options.color_esp_weapons);
        g_pSurface->DrawSetTextPos((bbox.left + w * 0.5f) - tw * 0.5f, bbox.bottom + 1);
        g_pSurface->DrawPrintText(buf, wcslen(buf));
    }
}
//--------------------------------------------------------------------------------
void Visuals::Misc::RenderDefuseKit(C_BaseEntity* ent)
{
    ...
    g_pSurface->DrawSetColor(g_Options.color_esp_defuse);
    g_pSurface->DrawLine(bbox.left, bbox.bottom, bbox.left, bbox.top);
    g_pSurface->DrawLine(bbox.left, bbox.top, bbox.right, bbox.top);
    g_pSurface->DrawLine(bbox.right, bbox.top, bbox.right, bbox.bottom);
    g_pSurface->DrawLine(bbox.right, bbox.bottom, bbox.left, bbox.bottom);
...
void Visuals::Misc::RenderPlantedC4(C_BaseEntity* ent)
{
    ...
    g_pSurface->DrawSetColor(g_Options.color_esp_c4);
    g_pSurface->DrawLine(bbox.left, bbox.bottom, bbox.left, bbox.top);
    g_pSurface->DrawLine(bbox.left, bbox.top, bbox.right, bbox.top);
    g_pSurface->DrawLine(bbox.right, bbox.top, bbox.right, bbox.bottom);
    g_pSurface->DrawLine(bbox.right, bbox.bottom, bbox.left, bbox.bottom);
