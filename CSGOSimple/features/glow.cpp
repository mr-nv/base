...
Glow::~Glow()
{
    ...
    //   exiting (because we try to access g_pGlowObjectManager).
    //
}
...
void Glow::Shutdown()
{
    ...
    for(auto i = 0; i < g_pGlowObjectManager->m_GlowObjectDefinitions.Count(); i++) {
        auto& glowObject = g_pGlowObjectManager->m_GlowObjectDefinitions[i];
...
void Glow::Run()
{
    for(auto i = 0; i < g_pGlowObjectManager->m_GlowObjectDefinitions.Count(); i++) {
        ...
        switch(class_id) {
            ...
            {
                auto is_enemy = entity->m_iTeamNum() != pLocal->m_iTeamNum();
