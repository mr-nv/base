...
namespace Hooks
{
    ...
    void Initialize()
    {
        hlclient_hook.setup(g_pClient);
        direct3d_hook.setup(g_pD3DDevice9);
        vguipanel_hook.setup(g_pPanel);
        vguisurf_hook.setup(g_pSurface);
        mdlrender_hook.setup(g_pMDLRender);
        clientmode_hook.setup(g_pClientMode);
    ...
    long __stdcall hkEndScene(IDirect3DDevice9* device)
    {
        ...
        static auto viewmodel_fov       = g_pCVar->FindVar("viewmodel_fov");
        static auto mat_ambient_light_r = g_pCVar->FindVar("mat_ambient_light_r");
        static auto mat_ambient_light_g = g_pCVar->FindVar("mat_ambient_light_g");
        static auto mat_ambient_light_b = g_pCVar->FindVar("mat_ambient_light_b");
        static auto crosshair_cvar      = g_pCVar->FindVar("crosshair");
    ...
    void __stdcall hkCreateMove(int sequence_number, float input_sample_frametime, bool active, bool& bSendPacket)
    {
		...
		/* GetAsyncKeyState -> InputSys::Get().IsKeyDown */
    ...
    void __stdcall hkPaintTraverse(vgui::VPANEL panel, bool forceRepaint, bool allowForce)
    {
        ...
        oPaintTraverse(g_pPanel, panel, forceRepaint, allowForce);
    ...
    void __stdcall hkPlaySound(const char* name)
    {
        ...
        oPlaySound(g_pSurface, name);

        // Auto Accept
        if(strstr(name, "UI/competitive_accept_beep.wav")) {
            static auto fnAccept =
                (void(*)())Utils::PatternScan("client.dll", "55 8B EC 83 E4 F8 83 EC 08 56 8B 35 ? ? ? ? 57 83 BE");
    ...
    int __stdcall hkDoPostScreenEffects(int a1)
    {
        ...
        if(pLocal && g_Options.glow_enabled)
            Glow::Get().Run();

        return oDoPostScreenEffects(g_pClientMode, a1);
    }
    //--------------------------------------------------------------------------------
    ...
    void __stdcall hkDrawModelExecute(IMatRenderContext* ctx, const DrawModelState_t& state, const ModelRenderInfo_t& pInfo, matrix3x4_t* pCustomBoneToWorld)
    {
        ...
        ofunc(g_pMDLRender, ctx, state, pInfo, pCustomBoneToWorld);
